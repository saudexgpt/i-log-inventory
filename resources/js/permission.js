...
const unrestricted_routes = ['/reports/bin-card', '/reports/graphical-reports', '/reports/downloads'];
        ...
        if (to.path === '/login') {
          ...
          if (hasRoles) {
            if (to.path !== '/subscribe') {
              if (unrestricted_routes.includes(to.path)) {
                next();
              } else {
                const subscription_status = await store.dispatch('subscribe/needToSubscribe');
                if (subscription_status === 'subscription needed') {
                // make sure client subscribes
                  next({ path: '/subscribe' });
                  NProgress.done();
                } else {
                  next();
